(function (f, m) {
  ...
  var It = (f, m, P) =>
    m in f
      ? Ct(f, m, { enumerable: !0, configurable: !0, writable: !0, value: P })
      : (f[m] = P);
  var a = (f, m, P) => (It(f, typeof m != "symbol" ? m + "" : m, P), P);
  class m {
    constructor(e) {
      a(this, "x");
      a(this, "y");
  ...
  class O {
    constructor(e) {
      ...
      (this.event = e),
        ...
        (this.viewName = e.viewName),
        (this.dataName = e.dataName),
        (this.dataValue = e.dataValue);
    }
  }
  function Ne(o) {
    return o
  ...
  function Ce(o) {
    return o
      .split("")
      .map((e, t) =>
        e.toUpperCase() === e ? `${t !== 0 ? "-" : ""}${e.toLowerCase()}` : e
      )
      .join("");
  }
  class k {
      ...
      e.forEach((t) => {
        t.addedNodes.forEach((n) => {
          if (
            ...
            n.dataset.velViewId ||
            (n.parentElement && typeof n.parentElement.dataset.velAdded < "u")
          )
            return;
          let r = n;
          if (
            (n.dataset.velView ||
              (r = n.querySelector("[data-vel-view][data-vel-plugin]")),
            !r)
          )
            return;
          ...
          u.length &&
            u.forEach((h) => {
              this._eventBus.emitEvent(k, { node: h });
            });
        }),
          t.removedNodes.forEach((n) => {
            if (
              !(n instanceof HTMLElement) ||
              typeof n.dataset.velProcessing < "u"
            )
              return;
            const r = n.querySelectorAll("[data-vel-plugin]");
            r.length &&
              r.forEach((u) => {
                this._eventBus.emitEvent(z, { node: u });
              }),
        ...
        if (i && /data-vel-data-.+/gi.test(i)) {
          const n = t.target,
            r = n.dataset.velPluginId || "",
            u = n.dataset.velPlugin || "",
            h = n.dataset.velView || "",
            c = n.getAttribute(i);
          if (c && c !== t.oldValue) {
            const g = Ne(i.replace("data-vel-data-", ""));
            this._eventBus.emitEvent(O, {
              pluginId: r,
              pluginName: u,
              viewName: h,
              dataName: g,
  ...
  class L {
    constructor() {
      a(this, "_listeners", new Map());
      a(this, "_keyedListeners", new Map());
    }
    subscribeToEvent(e, t, i) {
      ...
      s || ((s = []), this._listeners.set(e, s)), s.push(new ie(t));
    }
    _subscribeToKeyedEvent(e, t, i) {
      let s = this._keyedListeners.get(e);
      s || ((s = new Map()), this._keyedListeners.set(e, s));
      let n = s.get(i);
      n || ((n = []), s.set(i, n)), n.push(new ie(t));
    }
    emitEvent(e, t, i) {
      if (i) {
    ...
    subscribeToPluginReadyEvent(e, t, i = !1) {
      if (i) {
        this.subscribeToEvent(de, this._convertListener(e), t);
        return;
      }
      this.subscribeToEvent(ce, this._convertListener(e), t);
    }
    emitPluginReadyEvent(e, t, i = !1) {
      if (i) {
        this.emitEvent(de, t, e);
        return;
      }
      this.emitEvent(ce, t, e);
    }
    reset() {
  ...
  class ne {
    constructor(e, t, i, s, n, r, u) {
      ...
      a(this, "_isReady", !1);
      (this._id = se()),
        (this._pluginFactory = e),
        (this._pluginName = t),
        (this._registry = i),
        (this._eventBus = s),
        (this._appEventBus = n),
        (this._internalEventBus = new L()),
        (this._config = r),
    ...
    getViews(e) {
      return e
        ? this._registry.getViewsByNameForPlugin(this, e)
        : this._registry.getViewsForPlugin(this);
    }
    getView(e) {
      return e
        ? this._registry.getViewsByNameForPlugin(this, e)[0]
        : this._registry.getViewsForPlugin(this)[0];
    }
    getViewById(e) {
    ...
    useEventPlugin(e, t = {}) {
      const i = this._registry.createPlugin(e, this._eventBus, t);
    ...
    onDataChanged(e) {}
    removeView(e) {
      e.onRemoveCallback ? this._invokeRemoveCallback(e) : this._deleteView(e),
        this.onViewRemoved(e);
    }
    _invokeRemoveCallback(e) {
      const t = this._createTemporaryView(e);
      requestAnimationFrame(() => {
        var i;
        (i = t.onRemoveCallback) == null ||
          i.call(t, t, () => {
            var s, n;
            if (
              (s = e.onAddCallbacks) != null &&
              s.afterRemoved &&
              e.layoutId
            ) {
              const r = this._layoutIdViewMapWaitingToEnter.get(e.layoutId);
              (n = r == null ? void 0 : r.onAddCallbacks) == null ||
                n.afterEnter(r),
                this._layoutIdViewMapWaitingToEnter.delete(e.layoutId);
            }
    ...
    _deleteView(e, t = !1) {
      (t || !e.layoutId) &&
        (this._registry.removeViewById(e.id, this.id), e.element.remove());
    }
    _createTemporaryView(e) {
      const t = e.previousRect.viewportOffset,
        i = e.previousRect.size,
        s =
          e.rotation.degrees < 0
            ? 0
            : Math.sin(e.rotation.radians) * i.height * e.scale.y,
        n =
          e.rotation.degrees > 0
            ? 0
            : Math.sin(e.rotation.radians) * i.width * e.scale.y,
        r = e.element.cloneNode(!0);
      e.element.remove(),
        (r.style.cssText = ""),
        (r.style.position = "absolute"),
        (r.style.left = `${t.left + s}px`),
        (r.style.top = `${t.top - n}px`),
    ...
    `),
        ...
        (r.dataset.velRemoved = ""),
        document.body.appendChild(r);
      const u = this._registry.createView(r, e.name);
      return (
        u.setAsTemporaryView(),
        (u.styles.position = "absolute"),
        (u.styles.left = `${t.left + s}px`),
        (u.styles.top = `${t.top - n}px`),
        u.rotation.setDegrees(e.rotation.degrees, !1),
        u.scale.set({ x: e.scale.x, y: e.scale.y }, !1),
        u.size.set({ width: e._localWidth, height: e._localHeight }, !1),
        e._copyAnimatorsToAnotherView(u),
        e.onRemoveCallback && u.onRemove(e.onRemoveCallback),
        u
      );
    }
    onViewRemoved(e) {}
    ...
    _invokeAddCallbacks(e) {
      var t, i, s;
      (!((t = e.onAddCallbacks) != null && t.onInitialLoad) &&
        !this._initialized) ||
        ((i = e.onAddCallbacks) == null || i.beforeEnter(e),
        !((s = e.onAddCallbacks) != null && s.afterRemoved) ||
        !this._initialized
          ? requestAnimationFrame(() => {
              var n;
          ...
          : e.layoutId &&
            this._layoutIdViewMapWaitingToEnter.set(e.layoutId, e));
    }
    onViewAdded(e) {}
    init() {
      !this._initialized &&
        this._isReady &&
        (this.setup(), (this._initialized = !0));
    }
    setup() {}
    subscribeToEvents(e) {}
  }
  ...
  function W(o, e, t, i, s, n) {
    if (Se(o)) return new o(o, o.pluginName, e, t, i, s, n);
    const r = new re(o, o.pluginName, e, t, i, s, n),
      u = new ae(r);
    return o(u), r;
  }
  function Se(o) {
    ...
    return (
      ((e = o.prototype) == null
        ? void 0
        : e.constructor.toString().indexOf("class ")) === 0
    );
  }
  class l {
    constructor(e, t) {
  ...
  function $e(o, e, t = !1) {
    ...
    for (let i = 0; i < o.length; i++) {
      if (t && !p(o[i].value, e[i].value)) return !1;
      if (o[i].value !== e[i].value) return !1;
    }
    return !0;
  }
  function ue(o, e) {
  ...
  class T {
    ...
    equals(e) {
      return (
        p(this.value.topLeft.value, e.value.topLeft.value) &&
        p(this.value.topRight.value, e.value.topRight.value) &&
        p(this.value.bottomRight.value, e.value.bottomRight.value) &&
        p(this.value.bottomLeft.value, e.value.bottomLeft.value)
      );
    }
    toCssPercentageForNewSize(e) {
      const t = this._convertToPercentage(this._topLeft, e),
        i = this._convertToPercentage(this._topRight, e),
        s = this._convertToPercentage(this._bottomLeft, e),
        n = this._convertToPercentage(this._bottomRight, e);
      return `${t.h} ${i.h} ${n.h} ${s.h} / ${t.v} ${i.v} ${n.v} ${s.v}`;
    }
    _convertToPercentage(e, t) {
      if (e.unit === "%") return { h: `${e.value}%`, v: `${e.value}%` };
      const i = (e.value / t.width) * 100,
        s = (e.value / t.height) * 100;
      return { h: `${i}%`, v: `${s}%` };
    }
  }
  function U(o) {
    const e = o.split(" ").map((i) => w(i)),
      t = { value: 0, unit: "", valueWithUnit: "0" };
    switch (e.length) {
      case 1:
        return new T(e[0], e[0], e[0], e[0]);
      case 2:
        return new T(e[0], e[1], e[0], e[1]);
      case 3:
        return new T(e[0], e[1], e[2], e[1]);
      case 4:
        return new T(e[0], e[1], e[3], e[2]);
      default:
        return new T(t, t, t, t);
    }
  }
  function Oe(o, e) {
    const t = r(o.topLeft, e),
      i = r(o.topRight, e),
      s = r(o.bottomLeft, e),
      n = r(o.bottomRight, e);
    return {
      v: { topLeft: t.v, topRight: i.v, bottomRight: n.v, bottomLeft: s.v },
      h: { topLeft: t.h, topRight: i.h, bottomRight: n.h, bottomLeft: s.h },
    };
    function r(u, h) {
      if (u.unit === "%") return { h: w(`${u.value}%`), v: w(`${u.value}%`) };
      const c = (u.value / h.width) * 100,
        g = (u.value / h.height) * 100;
      return { h: w(`${c}%`), v: w(`${g}%`) };
    }
  }
  function le(o, e) {
    return (
      p(o.topLeft.value, e.topLeft.value) &&
      p(o.topRight.value, e.topRight.value) &&
      p(o.bottomRight.value, e.bottomRight.value) &&
      p(o.bottomLeft.value, e.bottomLeft.value)
    );
  }
  class ke {
    constructor(e) {
  ...
  function Ue(o, e) {
    const [t, i] = o.split(" "),
      s = w(t),
      n = w(i);
    return new We(s.value / e.width, n.value / e.height);
  }
  function qe(o) {
    const e = Ye(o),
      t = Xe(o);
    return {
      ...
      pageOffset: { top: t.top, left: t.left },
  ...
  function Xe(o) {
    let e = o,
      t = 0,
      i = 0;
    for (; e; ) (t += e.offsetTop), (i += e.offsetLeft), (e = e.offsetParent);
    return { top: t, left: i };
  }
  class je {
    constructor(e) {
      ...
      (this._rect = qe(e)),
        (this._computedStyle = getComputedStyle(e)),
        (this._element = e);
    }
    read(e) {
      switch (e) {
    ...
    get opacity() {
      return ze(this._computedStyle.opacity);
    }
    get borderRadius() {
      return U(this._computedStyle.borderRadius);
    }
    get origin() {
      return Ue(this._computedStyle.transformOrigin, this._rect.size);
    }
    get scroll() {
      let e = this._element,
        t = 0,
        i = 0;
      for (; e; ) (t += e.scrollTop), (i += e.scrollLeft), (e = e.offsetParent);
  ...
  function Y(o, e) {
    const t = {
      ...
      get: (i, s) =>
        typeof i[s] == "object" && i[s] !== null ? Y(i[s], e) : i[s],
    };
  ...
  class He extends j {
    update({ animatorProp: e, current: t, target: i, dt: s }) {
      const n = l.sub(i, t),
        r = l.scale(n, this._config.speed);
      let u = l.add(t, l.scale(r, s));
      return (
        this._shouldFinish(i, t, r) &&
          ((u = i),
          requestAnimationFrame(() => {
    ...
    _shouldFinish(e, t, i) {
      return l.sub(e, t).magnitude < E && i.magnitude < E;
    }
  }
  class Ke extends j {
    update({ animatorProp: e, current: t, target: i, dt: s }) {
    ...
    _shouldFinish(e, t, i) {
      return Math.abs(e - t) < E && Math.abs(i) < E;
    }
  }
  class Ge extends j {
    update({ animatorProp: e, current: t, target: i, dt: s }) {
      return i.map((n, r) => {
        const u = t[r],
          h = n.value === 0 ? u.unit : n.unit,
          g = (n.value - u.value) * this._config.speed,
          _ = u.value + g * s;
        let d = w(`${_}${h}`);
        return (
          this._shouldFinish(n.value, u.value, g) &&
            ((d = n),
            requestAnimationFrame(() => {
              e.callCompleteCallback();
            })),
  ...
  const K = { stiffness: 0.5, damping: 0.75, speed: 10 },
    x = 0.01;
  class G {
    constructor(e) {
      a(this, "name", "spring");
  ...
  class Ze extends G {
    ...
    update({ animatorProp: t, current: i, target: s, dt: n }) {
      const r = l.scale(l.scale(l.sub(i, s), -1), this._config.stiffness);
      (this._velocity = l.add(this._velocity, r)),
        (this._velocity = l.scale(this._velocity, this._config.damping));
      let u = l.add(i, l.scale(this._velocity, n * this._config.speed));
      return (
        this._shouldFinish(s, i) &&
          ((u = s),
          requestAnimationFrame(() => {
    ...
    _shouldFinish(t, i) {
      return l.sub(t, i).magnitude < x && this._velocity.magnitude < x;
    }
  }
  class Je extends G {
    constructor() {
    ...
    _shouldFinish(t, i) {
      return Math.abs(t - i) < x && Math.abs(this._velocity) < x;
    }
  }
  class Qe extends G {
    constructor() {
    ...
    update({ animatorProp: e, initial: t, target: i, ts: s }) {
      ...
      return p(n, 1)
        ? (requestAnimationFrame(() => {
          ...
          i)
        : l.add(t, l.scale(l.sub(i, t), this._config.ease(n)));
    }
  }
  class it extends J {
    update({ animatorProp: e, initial: t, target: i, ts: s }) {
      ...
      return p(n, 1)
        ...
        : t.map((r, u) => {
            const h = i[u],
              c = h.value === 0 ? r.unit : h.unit,
              g = r.value + this._config.ease(n) * (i[u].value - r.value);
            return w(`${g}${c}`);
          });
    }
  }
  ...
  class rt {
    ...
    callCompleteCallback() {
      var e;
      (e = this._completeCallback) == null || e.call(this),
        (this._isAnimating = !1);
    }
    onUpdate(e) {
  ...
  class R {
    constructor(e, t, i) {
      ...
      (this._animatorProp = new rt(this)),
        (this._animatorFactory = e),
        (this._initialValue = A(t)),
        (this._previousValue = A(t)),
        (this._targetValue = A(t)),
        (this._currentValue = A(t)),
        (this._hasChanged = !1),
        (this._previousRenderValue = void 0),
        (this._view = i),
    ...
    setAnimator(e, t) {
      this._animator = this._animatorFactory.createAnimatorByName(e, t);
    }
    _setTarget(e, t = !0) {
      var i, s;
      (this._previousValue = A(this._currentValue)),
        (this._targetValue = e),
        t
          ? ((s = (i = this._animator).reset) == null || s.call(i),
  ...
  class at extends R {
    ...
    set(t, i = !0) {
      ...
      const n = s.topLeft ? w(s.topLeft) : this._currentValue[0],
        r = s.topRight ? w(s.topRight) : this._currentValue[1],
        u = s.bottomRight ? w(s.bottomRight) : this._currentValue[2],
        h = s.bottomLeft ? w(s.bottomLeft) : this._currentValue[3];
    ...
    update(t, i) {
      if (this._forceStyleUpdateThisFrame)
        ...
        this._hasChanged = !0;
      else if (ue(this._targetValue, this._currentValue)) {
        this._hasChanged = !ue(this._targetValue, this._initialValue);
        return;
      }
    ...
    _applyScaleInverse() {
      if (p(this._view.scale.x, 1) && p(this._view.scale.y, 1)) return;
      const t = this._rect.size.width * this._view.scale.x,
        i = this._rect.size.height * this._view.scale.y;
      this._invertedBorderRadius = Oe(
        U(
    ...
    get shouldRender() {
      return this._hasChanged
        ? this._previousRenderValue
          ? !(
              le(this.renderValue.v, this._previousRenderValue.v) &&
              le(this.renderValue.h, this._previousRenderValue.h)
            )
          : !0
        : !1;
    }
    ...
    projectStyles() {
      const t = this.renderValue,
        i = `border-radius: ${t.h.topLeft.valueWithUnit} ${t.h.topRight.valueWithUnit} ${t.h.bottomRight.valueWithUnit} ${t.h.bottomLeft.valueWithUnit} / ${t.v.topLeft.valueWithUnit} ${t.v.topRight.valueWithUnit} ${t.v.bottomRight.valueWithUnit} ${t.v.bottomLeft.valueWithUnit};`;
  ...
  class ot extends R {
    ...
    get shouldRender() {
      return this._hasChanged
        ? typeof this._previousRenderValue > "u"
          ? !0
          : this.renderValue !== this._previousRenderValue
        : !1;
    }
    get renderValue() {
      return this.value;
  ...
  class ut extends R {
    ...
    get shouldRender() {
      ...
      const e = this.renderValue;
      return !(
        p(e.x, this._previousRenderValue.x) &&
        p(e.y, this._previousRenderValue.y)
      );
    }
    get renderValue() {
  ...
  class lt extends R {
    ...
    progressTo(t) {
      const i = typeof t.x > "u" ? this.initialX : t.x,
        s = typeof t.y > "u" ? this.initialY : t.y,
        n = new l(i, s),
        r = new l(this.initialX, this.initialY),
        u = new l(this.x, this.y),
        h = l.sub(u, r),
        c = l.sub(n, r);
    ...
    update(t, i) {
      if (
        ...
        const c = this._view._parent,
          g = c ? c.scale.x : 1,
          _ = c ? c.scale.y : 1;
        (this._parentScaleInverse = new l(1 / g, 1 / _)),
          this._parentScaleInverse.equals(new l(1, 1)) ||
            (this._hasChanged = !0);
      }
      if (
        ...
        return;
      const s = this._view._parent,
        n = s ? s.scale.x : 1,
        r = s ? s.scale.y : 1,
        u = s ? s.scale._previousValue.x : 1,
        h = s ? s.scale._previousValue.y : 1;
      (this._currentValue = this._animator.update({
        animatorProp: this._animatorProp,
        current: new l(this._currentValue.x * n, this._currentValue.y * r),
        target: this._targetValue,
        initial: new l(this._previousValue.x * u, this._previousValue.y * h),
        ts: t,
        dt: i,
      })),
    ...
    _runLayoutTransition() {
      const t = !(
        ...
        i = !(
          this._view.scale._targetValue.x ===
            this._view.scale._currentValue.x &&
          this._view.scale._targetValue.y === this._view.scale._currentValue.y
        ),
        s = t || i,
        n = this._rect.pageOffset.left - this._previousRect.pageOffset.left,
        r = this._rect.pageOffset.top - this._previousRect.pageOffset.top,
        u = this._previousRect.size.width / this._rect.size.width,
        h = this._previousRect.size.height / this._rect.size.height;
      let c = !1;
      if (
        (n !== 0 ||
        r !== 0 ||
        ...
        (!Number.isNaN(h) && h !== 1)
          ...
          : (c = (() => {
              ...
              for (let _ = 0; _ < g.length; _++) {
                const d = g[_],
                  v = d.previousRect.size.width / d.rect.size.width,
                  y = d.previousRect.size.height / d.rect.size.height;
                if (v !== 1 || y !== 1) return !0;
              }
              return !1;
            })()),
        ...
        if (
          ...
          if (!s) {
            const M =
              ...
              D = this._rect.pageOffset.top - this._previousRect.pageOffset.top;
            this._setTarget(
              new l(this._currentValue.x - M, this._currentValue.y - D),
              !1
            );
            return;
          }
          const V = this._view._parent,
            Ve = this._rect.pageOffset,
            Pe = this._view.getScroll(),
            F = {
              left: this._previousRect.viewportOffset.left + Pe.x,
              top: this._previousRect.viewportOffset.top + Pe.y,
            },
            xt = F.left - Ve.left,
            Tt = F.top - Ve.top;
          let be = 0,
            ...
            Ee = 0,
            xe = 0;
          if (V && V.position.animator.name !== "instant") {
            const M = V.rect.pageOffset,
              D = V.getScroll(),
              $ = {
                left: V.previousRect.viewportOffset.left + D.x,
                top: V.previousRect.viewportOffset.top + D.y,
              };
            (be = $.left - M.left), (Re = $.top - M.top);
            const Te = F.top - $.top,
              Ae = F.left - $.left,
              At = V.scale.y * Te;
            Ee = (Te - At) / V.scale.y;
            const Nt = V.scale.x * Ae;
            xe = (Ae - Nt) / V.scale.x;
          }
          this._setTarget(new l(xt - be + xe, Tt - Re + Ee), !1),
        ...
        const g = this._previousRect,
          _ = this._rect,
          d = this._view._parent;
        let v = 0,
          y = 0;
        d &&
          d.position.animator.name !== "instant" &&
          (v = d.previousRect.viewportOffset.left - d.rect.viewportOffset.left),
          d &&
            d.position.animator.name !== "instant" &&
            (y = d.previousRect.viewportOffset.top - d.rect.viewportOffset.top);
        let b = 1,
          te = 1;
        d &&
          d.scale.animator.name !== "instant" &&
          ((b = d.previousRect.size.width / d.rect.size.width),
          (te = d.previousRect.size.height / d.rect.size.height));
        const yt =
            d && d.position.animator.name !== "instant"
              ? d.previousRect.viewportOffset.left
              : 0,
          Vt =
            d && d.position.animator.name !== "instant"
              ? d.previousRect.viewportOffset.top
              : 0,
          we = g.viewportOffset.left - yt,
          ye = g.viewportOffset.top - Vt,
          Pt = we / b - we,
          bt = ye / te - ye;
        let Rt = g.viewportOffset.left - _.viewportOffset.left - v + Pt;
        const Et = g.viewportOffset.top - _.viewportOffset.top - y + bt;
        this._setTarget(new l(Rt, Et), !1);
    ...
    get shouldRender() {
      ...
      const t = this.renderValue;
      return !(
        p(t.x, this._previousRenderValue.x) &&
        p(t.y, this._previousRenderValue.y)
      );
    }
    get renderValue() {
      let t = 0,
        ...
        (this._view.isInverseEffectEnabled ||
          ...
          (i =
            (this._rect.size.height *
              ...
              this._rect.size.height) *
            this._view.origin.y)),
        new l(this._currentValue.x + t, this._currentValue.y + i)
      );
    }
    projectStyles() {
      const t = this.renderValue,
        i = `translate3d(${t.x}px, ${t.y}px, 0)`;
      return (this._previousRenderValue = t), i;
    }
    isTransform() {
      return !0;
  ...
  class ct extends R {
    ...
    set(t, i = !0) {
      const r = {
        ...{ x: this._currentValue.x, y: this._currentValue.y },
        ...(typeof t == "number" ? { x: t, y: t } : t),
      };
      this._setTarget(new l(r.x, r.y), i);
    }
    setWithSize(t, i = !0) {
      let s = this._currentValue.x,
        n = this._currentValue.y;
      t.width && (s = t.width / this._rect.size.width),
        t.height && (n = t.height / this._rect.size.height),
        !t.width && t.height && (s = n),
        !t.height && t.width && (n = s);
      const r = { x: s, y: n };
      this._setTarget(new l(r.x, r.y), i);
    }
    reset(t = !0) {
    ...
    _runLayoutTransition() {
      const t = !(
        ...
        i = this._previousRect.size.width / this._rect.size.width,
        s = this._previousRect.size.height / this._rect.size.height;
      let n = !1;
      if (
        (((!Number.isNaN(i) && i !== 1) || (!Number.isNaN(s) && s !== 1)) &&
          (n = !0),
        n)
      ) {
        ...
        const r = this._previousRect.size.width / this._rect.size.width,
          u = this._previousRect.size.height / this._rect.size.height,
          h = r,
          c = u;
        this._view.viewProps.borderRadius.applyScaleInverse(),
          this._setTarget(new l(h, c), !1),
          (this._animateLayoutUpdateNextFrame = !0);
      } else this._animateLayoutUpdateNextFrame && (this._setTarget(this._initialValue, !0), (this._animateLayoutUpdateNextFrame = !1));
    }
    get shouldRender() {
    ...
    get renderValue() {
      const t = this._view._parent ? this._view._parent.scale.x : 1,
        i = this._view._parent ? this._view._parent.scale.y : 1,
        s = this._currentValue.x / t,
        n = this._currentValue.y / i;
      return new l(s, n);
    }
    projectStyles() {
      const t = this.renderValue,
  ...
  class dt extends R {
    ...
    set(e, t = !0) {
      const s = {
        ...{ width: this._currentValue.x, height: this._currentValue.y },
        ...e,
      };
    ...
    reset(e = !0) {
      this._setTarget(new l(this.initialWidth, this.initialHeight), e);
    }
    update(e, t) {
      (this._targetValue.x === this._currentValue.x &&
        this._targetValue.y === this._currentValue.y) ||
  ...
  class gt {
    constructor(e) {
      a(this, "_props", new Map());
      this._props.set("position", new lt(new S(), new l(0, 0), e)),
        this._props.set("scale", new ct(new S(), new l(1, 1), e)),
        this._props.set("rotation", new ht(new he(), 0, e)),
        this._props.set(
          "size",
          new dt(new S(), new l(e.rect.size.width, e.rect.size.height), e)
        ),
        this._props.set(
          "opacity",
          new ot(new he(), e.elementReader.opacity.value, e)
        ),
        this._props.set(
          "borderRadius",
          new at(
        ...
        this._props.set(
          "origin",
          new ut(new S(), e.elementReader.origin.value, e)
        );
    }
    allProps() {
  ...
  class _t {
    constructor(e, t, i, s) {
      ...
      (this.id = se()),
        (this.name = t),
        (this.element = e),
        (this._elementReader = q(e)),
        (this._previousRect = this._elementReader.rect),
        (this._viewProps = new gt(this)),
        (this._skipFirstRenderFrame = !0),
        (this._layoutId = s),
        (this._layoutTransition = !1),
        (this._registry = i),
        (this.element.dataset.velViewId = this.id),
        (this._temporaryView = !1),
        (this._inverseEffect = !1),
        (this.styles = Y(this.styles, () => {
          this._renderNextTick = !0;
        })),
        (this._renderNextTick = !1);
    }
    destroy() {
      this._viewProps.allProps().forEach((e) => e.destroy()),
    ...
    setElement(e) {
      (this.element = e),
        (this._elementReader = q(this.element)),
        (this.element.dataset.velViewId = this.id);
    }
    get layoutId() {
    ...
    get _children() {
      return Array.from(this.element.children)
        .map((t) => t.dataset.velViewId)
        .filter((t) => t && typeof t == "string")
        .map((t) => this._registry.getViewById(t))
        .filter((t) => !!t);
    }
    get _parent() {
      var i;
    ...
    get _parents() {
      ...
      for (t = t.closest("[data-vel-view-id]"); t; ) {
        ...
        t =
          (i = t.parentElement) == null
            ? void 0
            : i.closest("[data-vel-view-id]");
      }
      return e;
    }
    get rotation() {
    ...
    get data() {
      ...
      return Object.keys(e)
        .filter((s) => s.includes("velData"))
        .map((s) => s.replace("velData", ""))
    ...
    setAnimatorsFromParent() {
      let e = this._parent;
      for (; e && !e._inverseEffect; ) e = e._parent;
      if (e) {
    ...
    intersects(e, t) {
      ...
      return (
        e >= i.x &&
        e <= i.x + this.size.width &&
        t >= i.y &&
        t <= i.y + this.size.height
      );
    }
    overlapsWith(e) {
      const t = e._localWidth * e.scale.x,
        i = e._localHeight * e.scale.y,
        s = this._localWidth * this.scale.x,
        n = this._localHeight * this.scale.y;
      return (
        this.position.x < e.position.x + t &&
        this.position.x + s > e.position.x &&
        this.position.y < e.position.y + i &&
        this.position.y + n > e.position.y
      );
    }
    distanceTo(e) {
      const t = new l(this.position.x, this.position.y),
        i = new l(e.position.x, e.position.y);
    ...
    get shouldRender() {
      return (
        ...
        this._viewProps.allProps().some((e) => e.shouldRender)
      );
    }
    render() {
      ...
      let e = "";
      const t = this._viewProps.allProps(),
        i = t.filter((n) => n.isTransform()),
        s = t.filter((n) => !n.isTransform());
      if (i.some((n) => n.hasChanged())) {
        const n = i.reduce(
          (r, u, h) => (
            (r += u.projectStyles()), h === i.length - 1 && (r += ";"), r
          ),
          "transform: "
        );
        e += n;
      ...
      s.forEach((n) => {
        ...
        (e += this._getUserStyles()),
        (this.element.style.cssText = e),
        (this._renderNextTick = !1);
    }
    _getUserStyles() {
      return Object.keys(this.styles).reduce(
        (e, t) => (t ? e + `${Ce(t)}: ${this.styles[t]};` : e),
        ""
      );
    }
    markAsAdded() {
    ...
    _copyAnimatorsToAnotherView(e) {
      e.viewProps.allPropNames().forEach((t) => {
        var s, n;
        const i =
          (s = this.viewProps.getPropByName(t)) == null
            ? void 0
            : s.getAnimator();
        i &&
          ((n = e.viewProps.getPropByName(t)) == null ||
            n.setAnimator(i.name, i.config));
      });
    }
  }
  class pt {
    constructor(e, t) {
      ...
      a(this, "_plugins", []);
      a(this, "_views", []);
      a(this, "_viewsPerPlugin", new Map());
      a(this, "_viewsToBeCreated", []);
      a(this, "_viewsToBeRemoved", []);
      a(this, "_viewsCreatedInPreviousFrame", []);
      a(this, "_layoutIdToViewMap", new Map());
      a(this, "_eventPluginsPerPlugin", new Map());
      a(this, "_pluginNameToPluginFactoryMap", new Map());
      a(this, "_pluginNameToPluginConfigMap", new Map());
    ...
    _isScopedElement(e) {
      const t = this._getPluginNameForElement(e);
      if (!t) return !1;
      const i = this._pluginNameToPluginFactoryMap.get(t),
    ...
    _handleAddedViews() {
      ...
      const e = this._viewsToBeCreated.filter((s) => this._isScopedElement(s)),
        t = this._viewsToBeCreated.filter((s) => !this._isScopedElement(s));
      (this._viewsToBeCreated = []),
        e.forEach((s) => {
          const n = this._getPluginNameForElement(s),
            r = this._pluginNameToPluginFactoryMap.get(n),
            u = this._pluginNameToPluginConfigMap.get(n),
            h = s.dataset.velPluginKey,
            c = W(r, this, this._eventBus, this._appEventBus, u, h);
          this._plugins.push(c);
          const g = s.dataset.velView,
            _ = this._createNewView(s, g, c);
          _.isInverseEffectEnabled && _.setAnimatorsFromParent(),
            c.notifyAboutViewAdded(_);
        });
      const i = t.filter((s) => !!this._getPluginIdForElement(s));
      i.length !== 0 &&
        i.forEach((s) => {
          const n = this._getPluginIdForElement(s),
            r = s.dataset.velView;
          if (!r || !n) return;
          ...
          let c;
          h && this._layoutIdToViewMap.has(h)
            ? ((c = this._layoutIdToViewMap.get(h)),
              c.setElement(s),
              c.setPluginId(u.id),
              this._createChildrenForView(c, u))
            : (c = this._createNewView(s, r, u)),
            c.isInverseEffectEnabled && c.setAnimatorsFromParent(),
            u.notifyAboutViewAdded(c);
        });
    }
    ...
    _createNewView(e, t, i) {
      const s = this._getLayoutIdForElement(e, i),
        n = this.createView(e, t, s);
      return (
        i.addView(n),
        n.layoutId && this._layoutIdToViewMap.set(n.layoutId, n),
        this._createChildrenForView(n, i),
        this._appEventBus.emitPluginReadyEvent(i.pluginName, i.api, !0),
        requestAnimationFrame(() => {
          requestAnimationFrame(() => {
    ...
    _createChildrenForView(e, t) {
      ...
      if (i.length) {
        ...
        Array.from(i).forEach((s) => {
          const n = s,
            r = n.dataset.velView ? n.dataset.velView : `${e.name}-child`,
            u = this._getLayoutIdForElement(n, t),
            h = this.createView(n, r, u);
          u &&
            !this._layoutIdToViewMap.has(u) &&
            this._layoutIdToViewMap.set(u, h),
            t.addView(h),
            t.notifyAboutViewAdded(h);
        });
      }
    }
    _handleRemoveView(e) {
      this._plugins.forEach((t) => {
        if (!this._viewsPerPlugin.get(t.id)) return;
        const s = this._getPluginViewById(t, e);
        s && t.removeView(s);
      });
    }
    removeViewById(e, t) {
      this._unassignViewFromPlugin(e, t),
        (this._views = this._views.filter((i) => i.id !== e));
    }
    ...
    destroy(e, t) {
      ...
      if (e && e.length > 0) {
        ...
        if (s) {
          const r = (this._eventPluginsPerPlugin.get(s.id) || [])
            .map((u) => this._getPluginById(u))
            .filter((u) => typeof u < "u");
          i.push(s), i.push(...r);
        }
    ...
    _destroyPlugin(e) {
      ...
      t.forEach((i) => {
        ...
        this._viewsPerPlugin.delete(e.id),
        (this._plugins = this._plugins.filter((i) => i.id !== e.id));
    }
    _destroyAll(e) {
      this._views.forEach((t) => t.destroy()),
        requestAnimationFrame(() => {
          (this._plugins = []),
            (this._views = []),
    ...
    _resetPlugin(e) {
      const t = e.config,
        i = e.pluginFactory,
        s = e.internalBusEvent,
        n = !e.isRenderable(),
        r = this.getViewsForPlugin(e);
      r.forEach((u) => {
        ...
        this._viewsPerPlugin.delete(e.id),
        (this._plugins = this._plugins.filter((u) => u.id !== e.id)),
        n ||
          requestAnimationFrame(() => {
            this.createPlugin(i, this._eventBus, t).setInternalEventBus(s);
          });
    }
    ...
    notifyPluginAboutDataChange(e) {
      const t = this._plugins.filter((i) => i.id === e.pluginId);
      !t ||
        !t.length ||
        t.forEach((i) => {
    ...
    getPluginByName(e, t) {
      return this._plugins.find((i) =>
        t ? i.pluginKey === t && i.pluginName === e : i.pluginName === e
      );
    }
    getPluginsByName(e, t) {
      return this._plugins.filter((i) =>
        t ? i.pluginKey === t && i.pluginName === e : i.pluginName === e
      );
    }
    hasPlugin(e) {
    ...
    createPlugin(e, t, i = {}) {
      if (!e.pluginName)
        throw Error(`Plugin ${e.name} must contain a pluginName field`);
      let s = [];
      if (e.scope) {
        const u = document.querySelectorAll(
          `[data-vel-plugin=${e.pluginName}][data-vel-view=${e.scope}]`
        );
        this._pluginNameToPluginFactoryMap.has(e.pluginName) ||
          this._pluginNameToPluginFactoryMap.set(e.pluginName, e),
          this._pluginNameToPluginConfigMap.has(e.pluginName) ||
            this._pluginNameToPluginConfigMap.set(e.pluginName, i),
          u ? (s = Array.from(u)) : (s = [document.documentElement]);
      } else s = [document.documentElement];
      const n = s.map((u) => {
        const h = u.dataset.velPluginKey,
          c = W(e, this, t, this._appEventBus, i, h);
        this._plugins.push(c);
        let g = [];
        u !== document.documentElement && g.push(u);
        const _ = u.querySelectorAll(`[data-vel-plugin=${c.pluginName}]`);
        g = [...g, ..._];
        const d = g.filter((v) => {
          if (!v.parentElement) return !0;
          const b = this._getPluginNameForElement(v.parentElement);
          return !(b && b.length > 0);
        });
        return (
          d.length &&
            d.forEach((v) => {
              const y = v.dataset.velView;
              if (!y) return;
              const b = this._createNewView(v, y, c);
      ...
      const r = W(e, this, t, this._appEventBus, i);
      return (
        e.scope ||
          console.log(
    ...
    getViewsForPlugin(e) {
      ...
      return t
        ? t.map((s) => this._views.find((n) => n.id === s)).filter((s) => !!s)
        : [];
    }
    getViewsByNameForPlugin(e, t) {
      return this.getViewsForPlugin(e).filter((i) => i.name === t);
    }
  }
  class ce {
    constructor(e) {
  ...
  class ee {
    constructor() {
      ...
      (this._eventBus = new L()),
        (this._appEventBus = new L()),
        (this._registry = new pt(this._appEventBus, this._eventBus)),
    ...
    addPlugin(e, t = {}) {
      this._registry.hasPlugin(e) ||
        this._registry.createPlugin(e, this._eventBus, t);
    }
    reset(e, t) {
    ...
    _subscribeToEvents() {
      this._eventBus.subscribeToEvent(k, this._onNodeAdded.bind(this)),
        this._eventBus.subscribeToEvent(z, this._onNodeRemoved.bind(this)),
        this._eventBus.subscribeToEvent(O, this._onDataChanged.bind(this)),
  ...
  class ge {
    constructor(e) {
      ...
      a(this, "height");
      (this.props = e),
        (this.previousX = e.previousX),
        (this.previousY = e.previousY),
        (this.x = e.x),
        (this.y = e.y),
        (this.width = e.width),
  ...
  class _e extends B {
    constructor() {
      ...
      a(this, "_initialPointerPerView", new Map());
      a(this, "_pointerDownPerView", new Map());
      a(this, "_targetPerView", new Map());
      a(this, "_viewPointerPositionLog", new Map());
    }
    setup() {
    ...
    subscribeToEvents(t) {
      t.subscribeToEvent(C, ({ x: i, y: s, target: n }) => {
        (this._initialPointer = new l(i, s)),
          this.getViews().forEach((r) => {
            this._pointerDownPerView.set(r.id, r.intersects(i, s)),
              this._targetPerView.set(r.id, n);
            const u = new l(i - r.position.x, s - r.position.y);
            (this._pointerX = i),
              (this._pointerY = s),
              this._initialPointerPerView.set(r.id, u);
          });
      }),
        t.subscribeToEvent(I, () => {
          this.getViews().forEach((i) => {
            this._pointerDownPerView.get(i.id) &&
              this._initialPointerPerView.get(i.id) &&
              (this._pointerDownPerView.set(i.id, !1), this._emitEvent(i, []));
          });
        }),
        t.subscribeToEvent(N, ({ x: i, y: s }) => {
          (this._pointerX = i),
            (this._pointerY = s),
            this.getViews().forEach((n) => {
              if (
                this._pointerDownPerView.get(n.id) &&
                this._initialPointerPerView.get(n.id)
              ) {
                this._viewPointerPositionLog.has(n.id) ||
                  this._viewPointerPositionLog.set(n.id, []);
                const r = new l(i, s),
                  u = this._viewPointerPositionLog.get(n.id);
                u && u.push(new l(i, s));
                const h = u && u.length >= 2 ? u[u.length - 2] : r.clone(),
                  c = this._calculateDirections(h, r);
                this._emitEvent(n, c);
              }
            });
        });
    ...
    _emitEvent(t, i) {
      const s = this._viewPointerPositionLog.get(t.id),
        n = s && s.length >= 2 ? s[s.length - 2] : null,
        r = this._pointerX - this._initialPointerPerView.get(t.id).x,
        u = this._pointerY - this._initialPointerPerView.get(t.id).y,
        h = n ? n.x - this._initialPointerPerView.get(t.id).x : r,
        c = n ? n.y - this._initialPointerPerView.get(t.id).y : u,
        g = this._pointerY - this._initialPointer.y,
        _ = this._pointerX - this._initialPointer.x,
        d = this._targetPerView.get(t.id);
      if (!d || !t.hasElement(d)) return;
    ...
    _calculateDirections(t, i) {
      ...
      return [
        ...
        { direction: "down", projection: n.dot(s.down) },
        { direction: "left", projection: n.dot(s.left) },
        { direction: "right", projection: n.dot(s.right) },
      ]
        .filter((h) => h.projection > 0)
        .map((h) => h.direction);
    }
  }
  a(_e, "pluginName", "DragEventPlugin");
  class pe {
    constructor(e) {
      a(this, "view");
      a(this, "direction");
  ...
  class fe extends B {
    constructor() {
      super(...arguments);
      a(this, "_viewIsPointerDownMap", new Map());
      a(this, "_viewPointerPositionLog", new Map());
      a(this, "_targetPerView", new Map());
    }
    subscribeToEvents(t) {
      t.subscribeToEvent(C, ({ x: i, y: s, target: n }) => {
        this.getViews().forEach((r) => {
          this._targetPerView.set(r.id, n),
            r.intersects(i, s) && this._viewIsPointerDownMap.set(r.id, !0);
        });
      }),
        t.subscribeToEvent(N, ({ x: i, y: s }) => {
          this.getViews().forEach((n) => {
            if (!this._viewIsPointerDownMap.get(n.id)) return;
            this._viewPointerPositionLog.has(n.id) ||
              this._viewPointerPositionLog.set(n.id, []),
              this._viewPointerPositionLog.get(n.id).push(new l(i, s));
          });
        }),
        t.subscribeToEvent(I, ({ x: i, y: s }) => {
          this.getViews().forEach((r) => {
            if (
              !this._viewIsPointerDownMap.get(r.id) ||
              !this._viewPointerPositionLog.has(r.id)
            )
              return;
            const u = new l(i, s),
              h = this._viewPointerPositionLog.get(r.id),
              c = h[h.length - 2] || u.clone(),
              g = this._targetPerView.get(r.id),
              _ = n(c, u);
            g &&
              r.hasElement(g) &&
              _.hasSwiped &&
              this.emit(pe, { view: r, direction: _.direction }),
              this._viewPointerPositionLog.set(r.id, []),
              this._viewIsPointerDownMap.set(r.id, !1);
          });
          function n(r, u) {
            const h = {
              ...
              c = l.sub(u, r).unitVector,
              g = ["up", "down", "left", "right"],
              _ = [c.dot(h.up), c.dot(h.down), c.dot(h.left), c.dot(h.right)],
              d = Math.max(..._),
              v = _.indexOf(d),
              y = g[v],
              b = l.sub(u, r).magnitude;
            return { hasSwiped: c.dot(h[y]) * b > 30, direction: y };
          }
        });
    }
  }
  ...
  class ve extends B {
    subscribeToEvents(e) {
      e.subscribeToEvent(P, ({ x: t, y: i, target: s }) => {
        this.getViews().forEach((n) => {
          const r = s,
            u = n.element === r || n.element.contains(r);
          n.intersects(t, i) && u && this.emit(me, { view: n });
        });
      });
    }
  }
  a(ve, "pluginName", "ClickEventPlugin");
  function mt(o, e) {
    const t = o.map(e),
      i = Math.min(...t),
      s = t.indexOf(i);
      ...
      Object.defineProperty(
        {
          ...
          clamp: oe,
          minBy: mt,
          pointToViewProgress: De,
          remap: Me,
          valueAtPercentage: Fe,
        },
        Symbol.toStringTag,
        { value: "Module" }
      )
  ...
  (f.ClickEvent = me),
    (f.ClickEventPlugin = ve),
    (f.DataChangedEvent = O),
    (f.DragEvent = ge),
    (f.DragEventPlugin = _e),
    (f.EventBus = L),
    (f.EventPlugin = B),
    (f.Events = wt),
    (f.Plugin = re),
