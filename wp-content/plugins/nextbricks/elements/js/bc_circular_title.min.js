class NextArcTitle {
  ...
  constructor(element) {
    ...
    this.inner = element.querySelector(".next-arc-title__inner");
    this.options = {
      spacing: element.getAttribute("data-spacing") || 1,
      size: 1,
    };
    ...
    this.scrollRotation = parseFloat(
      element.getAttribute("data-scroll-rotation")
    );
  ...
  updateContent() {
    ...
    if (content) {
      ...
      this.inner.textContent = content;
    } else {
      this.options.text =
        this.inner.textContent || "Arc Title · Next Bricks · ";
    }
  }
  createArcText() {
    this.inner.innerHTML = "";
    const chars = this.options.text.split("");
    this.inner.style.setProperty("--char-count", chars.length);
    ...
    srSpan.textContent = this.options.text;
    this.inner.appendChild(srSpan);
  }
  updateStyles() {
    const { spacing, size } = this.options;
    const charCount = this.inner.children.length - 1;
    this.inner.style.setProperty("--font-size", `${size}rem`);
    this.inner.style.setProperty("--character-width", spacing);
    const radius = this.canTrig
      ? "calc((var(--character-width) / sin(var(--inner-angle))) * -1ch)"
      : `calc((${spacing} / ${Math.sin(
          360 / charCount / (180 / Math.PI)
        )}) * -1ch)`;
    this.inner.style.setProperty("--radius", radius);
    if (charCount > 3) {
  ...
  static destroyAll() {
    NextArcTitle.instances.forEach((instance) => {
      instance.destroy();
      NextArcTitle.instances.delete(instance);
    });
  }
  initScrollTrigger() {
    gsap.to(this.inner, {
      scrollTrigger: {
        ...
        scrub: this.scrub === "true" ? 1 : parseFloat(this.scrub) || !1,
      },
...
function next_arc_title() {
  NextArcTitle.destroyAll();
  document.querySelectorAll(".next-arc-title").forEach((element) => {
    const instance = new NextArcTitle(element);
    NextArcTitle.instances.add(instance);
  });
}
document.addEventListener("DOMContentLoaded", function (t) {
  bricksIsFrontend && next_arc_title();
});
