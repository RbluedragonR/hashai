function next_meteors() {
  class Meteor {
    constructor(options) {
      ...
      that.gradient.addColorStop(
        0,
        `hsl(${this.hue}, ${this.saturation}%, 100%)`
      );
      that.gradient.addColorStop(
        0.025,
        `hsl(${this.hue}, ${this.saturation}%, 70%)`
      );
      that.gradient.addColorStop(
        0.15,
        `hsl(${this.hue}, ${this.saturation}%, 40%)`
      );
      that.gradient.addColorStop(
        0.55,
        `hsl(${this.hue}, ${this.saturation}%, 20%)`
      );
    ...
    draw() {
      ...
      const visibleParticles = that.particles.filter(
        (p) => !p.dead && p.size > 0.5
      );
  ...
  document.querySelectorAll(".next-meteors").forEach((meteors) => {
    if (isBricks && meteors.dataset.disableBuilder === "1") return;
    function createMeteor(meteor) {
      var meteorWidth = meteor.getAttribute("data-width");
      var hue = meteor.getAttribute("data-hue");
      var saturation = meteor.getAttribute("data-saturation");
      new Meteor({
        element: meteor,
        width: parseFloat(meteorWidth),
        hue: parseFloat(hue),
    ...
    if (isBricks) {
          ...
          if (mutation.type === "childList") {
            mutation.addedNodes.forEach((node) => {
              if (
                ...
                node.classList.contains("next-meteors__meteor")
              ) {
                createMeteor(node);
              }
            });
      ...
      const config = { childList: !0, subtree: !0 };
      observer.observe(meteors, config);
    } else {
      meteors.querySelectorAll(".next-meteors__meteor").forEach(createMeteor);
    }
  });
}
document.addEventListener("DOMContentLoaded", function (t) {
  bricksIsFrontend && next_meteors();
});
